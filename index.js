...
;(function () {
    ...
    canvasProgress.prototype = {
        init: function init() {
            var _self = this;
            [].forEach.call(document.querySelectorAll(_self.str), function (item, index) {
        ...
        dataInit: function dataInit(canvas) {
            var _self = this;
            canvas.width = 300;
            canvas.height = 300;
            _self.percentArr.push(parseInt(canvas.dataset.process));
        ...
        inSdk: function inSdk() {
            return typeof window.androidtojs !== "undefined" ? true : false;
        },
        circle: function circle(ctx, cx, cy, r) {
            var _self = this;
            //画原始进度圆
            ctx.beginPath();
            //ctx.moveTo(cx + r, cy);
        ...
        sector: function sector(ctx, cx, cy, r, startAngle, endAngle) {
            var _self = this;
            ctx.beginPath();
            ctx.lineWidth = _self.lineWidth;
            ...
            _self.anti ? ctx.arc(cx, cy, r, (2.5 - 0.05 * _self.radio) * Math.PI, (2.5 - 0.05 * _self.radio - degRadio * endAngle) * Math.PI, _self.anti) : ctx.arc(cx, cy, r, (0.5 + 0.05 * _self.radio) * Math.PI, (0.5 + 0.05 * _self.radio + degRadio * endAngle) * Math.PI, _self.anti);
            ctx.stroke();
        },
        loading: function loading(ctx, index) {
            var _self = this;
            ...
            if (parseInt(_self.addprocessArr[index]) >= 100 - _self.percentArr[index]) {
                ...
                _self.circleLoading[index] = setTimeout(function () {
                    _self.loading(ctx, index);
                }, 20);
            }
        }
